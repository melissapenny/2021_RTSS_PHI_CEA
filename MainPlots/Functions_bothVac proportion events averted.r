...
EventsAvertedPlot_bothVac <- function(Data, my_Event, my_FigureNumber, my_time, xlab='', ylab='', main='', save_plots=T ) {
	...
	# set group colours
	groupColours = my_col# c('GSK'='#F8766D','EMOD DTK'='#7CAE00','Imperial'='#00BFC4','OpenMalaria'='#C77CFF')
...
# plot1 = plot1 + geom_bar(stat='identity',aes(x=(Age_lower+Age_upper)/2,y=median/(Age_upper-Age_lower),width=Age_upper-Age_lower,fill=factor(Group)),colour='white',size=0.2,position="dodge",data= data_withoutBoost_spread)
	...
	if(save_plots){
		tiff(paste(pathPlots,"Figure ",my_FigureNumber,"_proportionAverted.tif",sep=""), width=plot_width, height=plot_height, units="cm", pointsize=8, compression="lzw", res= resolution)
		print(plot1)
		dev.off()
	} else {
		print(plot1)
