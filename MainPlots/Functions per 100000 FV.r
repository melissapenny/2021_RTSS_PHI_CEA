...
EventsAvertedPlot <- function(Data, my_boost, my_Event, my_FigureNumber, my_time, save_plots=T ){
  ...
  # get confidence intervals in shape
	...
	# save file
		if(save_plots){
			tiff(paste(pathPlots,"Figure ",my_FigureNumber," ",my_boost,"_per100000FV.tif",sep=""), width=plot_width, height=plot_height, units="cm", pointsize=8, compression="lzw", res=resolution)
			print(my_plot)
			dev.off()
		}
		return(my_plot)
