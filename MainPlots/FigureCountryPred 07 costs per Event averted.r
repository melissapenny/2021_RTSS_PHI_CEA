...
#plot
	my_plot = ggplot(data=Data_tmp_CI, aes(x=Transmission ,y=median, ymin=Q0.025, ymax=Q0.975, group=factor(Vaccine_price), colour=Group, fill=factor(Vaccine_price))) +
				geom_point(stat="identity", size=2) + 
				geom_errorbar(width=0.2, size=1)+
				facet_grid(Event_lab ~ Group, scales="free_y") + 
				xlab('') + ylab("Cost in USD per events averted") + 
				theme_bw() + scale_fill_discrete(name="Vaccine price in USD") +
	      theme(axis.text.x = element_text(angle = 90, hjust = 1)) 

			
# save file
	#if(save_plots){
	#	tiff(paste(pathPlots,"FigureCountryPred 7 ",my_boost,".tif",sep=""), width=25, height=22, units="cm", pointsize=8, compression="lzw", res=resolution)
	#	print(my_plot)
	#	dev.off()
	#}
	...
	#plot
	my_plot = ggplot(data=Data_tmp_CI, aes(x=Transmission ,y=median, group=factor(Vaccine_price), colour= Group, shape=factor(Vaccine_price))) +
	  #geom_line(stat="identity") +
	  geom_point(stat="identity") + 
	  facet_grid(Event_lab ~ Group, scales="free_y") + 
	  xlab('') + ylab("Cost in USD per events averted") + 
	  theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + scale_shape_discrete(name="Vaccine price in USD") +
    scale_color_manual(name="Groups",values=my_col)    
	
# save file
	if(save_plots){
		tiff(paste(pathPlots,"FigureCountryPred 7 ",my_boost," noCI.tif",sep=""), width=23, height=18, units="cm", pointsize=8, compression="lzw", res=resolution)
		print(my_plot)
		dev.off()
	}
	

Data_tmp_CI_tab_beforeSubsetting = Data_tmp_CI_tab	
# write table on dalys averted
for(VacPrice in c(2,5,10)){
	  Data_tmp_CI_tab=subset(Data_tmp_CI_tab_beforeSubsetting, Event_lab %in% c("DALY (undiscounted)"))
    Data_tmp_CI_tab=subset(Data_tmp_CI_tab, Vaccine_price == VacPrice)
    ...
    write.csv(df,paste(pathPlots,"Table_costperdaly_",VacPrice,"_6countries_",my_boost,".csv",sep=""))
    ...
    write.csv(df,paste(pathPlots,"Table_costperdaly_",VacPrice,"_6countries_",my_boost,"_MediansOnly.csv",sep=""))
